...
def process_hand(
    ...
    try:
        ...
        if (
            np.all(hand_pose >= top_left_corner)
            and np.all(hand_pose <= bottom_right_corner)
            and hand_pose[2] > 0.0
        ):
            warning_area = True
        ...
        cv2.rectangle(
            color_image,
            (int(hand[0] - RECT_SIZE[0] / 2), int(hand[1] - RECT_SIZE[0] / 2)),
            (int(hand[0] + RECT_SIZE[1] / 2), int(hand[1] + RECT_SIZE[1] / 2)),
            (0, 255 * int(not warning_area), 255 * int(warning_area)),
            2,
        )
...
def main():
    ...
    while True:
        ...
        warning_area_left = process_hand(
            results[0].keypoints.xy[0][9].cpu().numpy(),
            depth_frame,
            camera_intrinsics,
            color_image,
            RECT_SIZE,
        ...
        warning_area_right = process_hand(
            results[0].keypoints.xy[0][10].cpu().numpy(),
            depth_frame,
            camera_intrinsics,
            color_image,
            RECT_SIZE,
        ...
        # draw a Warning text
        if warning_area_left or warning_area_right:
            cv2.putText(color_image, "WARNING", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)            
        ...
        # press 'c' to capture pointcloud
            ...
            xyz = (
                ...
                .view(np.float32)
                .reshape(camera_intrinsics.height * camera_intrinsics.width, 3)
            )
            ...
            bbox_line_set = o3d.geometry.LineSet.create_from_axis_aligned_bounding_box(
                bbox
            )
            bbox_line_set.colors = o3d.utility.Vector3dVector(
                [[1, 0, 0] for _ in range(len(bbox_line_set.lines))]
            )  # set color to red
            ...
            o3d.visualization.draw_geometries(
                ...
                zoom=0.2,
                front=[
                    -0.069559414540394313,
                    0.071052676612831794,
                    -0.99504422263281855,
                ],
